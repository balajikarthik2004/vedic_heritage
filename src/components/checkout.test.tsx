 ...
 *
 * A sponsorship enquiry is a conversation, not a checkout, so this button
 * shows the phone number and email address. It used to navigate to a `mailto:`
 * link - which does nothing at all on a machine with no mail client - so the
 * button looked broken to precisely the visitor worth the most to the
 * fundraiser.
 */
describe('Contact for sponsorship', () => {
  beforeEach(() => stubApi());
  ...
  async function clickContact() {
    ...
    await user.click(
      screen.getByRole('button', { name: /contact for sponsorship/i })
    );
    return { user, dialog: await screen.findByRole('dialog') };
  }

  it('shows the phone number and email address', async () => {
    const { dialog } = await clickContact();

    expect(within(dialog).getByText('+16318059105')).toBeInTheDocument();
    expect(within(dialog).getByText('vedic.heritageinc@gmail.com')).toBeInTheDocument();
    // Still offered, for anyone who does have mail configured.
    expect(within(dialog).getByRole('link', { name: /pre-filled enquiry/i })).toBeInTheDocument();
  });

  it('does not claim that payment has failed', async () => {
    // The wording matters: this visitor pressed a button to talk to us, and is
    // not being told bad news about their payment.
    const { dialog } = await clickContact();

    expect(within(dialog).getByRole('heading', { name: /talk to us about sponsorship/i })).toBeInTheDocument();
    expect(dialog).not.toHaveTextContent(/online payment is unavailable/i);
    expect(dialog).toHaveTextContent(/choose the right tier/i);
  });

  it('is dismissible', async () => {
    const { user } = await clickContact();

    await user.keyboard('{Escape}');
    await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument());
  });

  it('opens no checkout and calls no payment endpoint', async () => {
    const calls = stubApi();
    await clickContact();

    expect(calls.some((c) => c.url.endsWith('/payments/checkout'))).toBe(false);
    expect(screen.queryByRole('heading', { name: /confirm your sponsorship/i })).not.toBeInTheDocument();
  });
});
